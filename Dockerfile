...
RUN echo "http://dl-cdn.alpinelinux.org/alpine/edge/community" >> /etc/apk/repositories && \
    ...
    apk --no-cache update && \
    apk add --no-cache --virtual .build-deps gifsicle pngquant optipng git libjpeg-turbo-utils udev ttf-opensans && \
    apk add --no-cache python alpine-sdk chromium-chromedriver chromium xvfb udev yarn bash iptables ca-certificates openssh-client e2fsprogs docker wget curl bash && \
